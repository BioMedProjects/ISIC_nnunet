...
def predict_cases(
    ...
    for preprocessed in preprocessing:
        ...
        item = {
            ...
            'feature_vector': trainer.network.myBottleneck.cpu().data.numpy().tolist()
        }
        fv_data.append(item)
        if len(fv_data) % 100 == 0:
            with open("vectors/featureVector_{0}_{1}.json".format(iter, iter-100), "w+") as f:
                json.dump(fv_data, f, indent=2)
    ...
    # with open("featureVectorKW_-4.json" , "w+") as f:
    #     json.dump(fv_data, f, indent=2)
